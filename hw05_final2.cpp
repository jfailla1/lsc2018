...
int main(int argc, char **argv) {
	...
	#pragma omp parallel for
		for (int ii = i; ii < std::min(numOfParticles, i+ bsize); ii+=bsize) {
				...
				tr2 *= foc[ii * DIM] * dx + foc[ii * DIM + 1] * dy;

				vel[p * DIM] += -foc[ii * DIM] * tr1 + tr2 * dx;
				vel[p * DIM + 1] += -foc[ii * DIM + 1] * tr1 + tr2 * dy;	
		}
		}
	}
	double et = tsecond();
