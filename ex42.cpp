...
int main(int argc, char **argv) {
	...
	double *Sc = (double *)mkl_malloc(num * sizeof(double), 64);
	double *phiA = (double *)mkl_malloc(num * sizeof(double), 64);
#else
	double *phi = new double[num];
	double *Sc = new double[num];
	double *phiA = new double[num];
#endif
	// Setup
	double dx = 1.0 / (num - 1);
	for (int i = 0; i < num; i++) {
	...
	for (int i = 0; i < num; i++) {
		phi[i] = Sc[i]; /* use phi for RHS array */
		phiA[i] = (((M_E*dx*i)-(dx*i)+1.0-exp(dx*i))/d) + a + (dx*i)*(b-a);
	}
	phi[0] -= (a * aw) + Sc[0];
	phi[num - 1] -= (b * ae) + Sc[num - 1];
	...
	// Output Result
	std::ofstream ofile;
	ofile.open("resEx40.csv");
	ofile << std::setprecision(16);
	ofile << std::scientific;
	for (int i = 0; i < num; i++) {
		ofile << dx * i << "," << phi[i] << "," << phiA[i] << std::endl;
	}
	ofile.close();
