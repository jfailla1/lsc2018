...
int main(int argc, char **argv) {
	...
	double *vel_dev = new double[mysize];
        ...
        #pragma omp target if(dev != num_dev) device(dev) map(to:foc[mystart:myend]) map(to:loc[mystart:myend]) map(to:vel[mystart:myend]) map(from:vel_dev[0:mysize])
